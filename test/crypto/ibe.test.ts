...
import {fp12FromBigTwelve, fp2FromBigTuple} from "../../src/crypto/fp"
import {bls12_381} from "@noble/curves/bls12-381"


describe("fpToBytes", () => {
    it("should reverse the order of values in the Fps", () => {
        const combined = Buffer.concat([fpToBytes(1n), fpToBytes(2n)])
        const fp2 = fp2ToBytes(fp2FromBigTuple([2n, 1n]))

        expect(Buffer.compare(fp2, combined)).to.equal(0)
    })

    it("Fp2s should be reversed when combined to an Fp12", () => {
        const one = fp2ToBytes(fp2FromBigTuple([1n, 2n]))
        const two = fp2ToBytes(fp2FromBigTuple([3n, 4n]))
        const three = fp2ToBytes(fp2FromBigTuple([5n, 6n]))
        const four = fp2ToBytes(fp2FromBigTuple([7n, 8n]))
        const five = fp2ToBytes(fp2FromBigTuple([9n, 10n]))
        const six = fp2ToBytes(fp2FromBigTuple([11n, 12n]))
        const combined = Buffer.concat([one, two, three, four, five, six])

        const fullFp12 = fp12ToBytes(fp12FromBigTwelve([11n, 12n, 9n, 10n, 7n, 8n, 5n, 6n, 3n, 4n, 1n, 2n]))
    ...
    it("big nums should not lose precision", () => {
        ...
        const bytes = fpToBytes(1099511627775n)
    ...
    it("correctly pass the test vectors generated from the go codebase", () => {
        ...
        expect(bls12_381.pairing(bls12_381.G1.ProjectivePoint.BASE, bls12_381.G2.ProjectivePoint.BASE)).to.deep.equal(test)
        expect(fp12ToBytes(bls12_381.pairing(bls12_381.G1.ProjectivePoint.BASE, bls12_381.G2.ProjectivePoint.BASE))).to.deep.equal(fp12ToBytes(test))
        ...
        const actualG1Compressed = Buffer.from(bls12_381.G1.ProjectivePoint.BASE.toRawBytes(true)).toString("hex")
        ...
        const actualG2Uncompressed = Buffer.from(bls12_381.G2.ProjectivePoint.BASE.toRawBytes(false)).toString("hex")
